...
VALUES (DATE(CONVERT_TZ(NOW(),'SYSTEM','-05:00')),'INGRESO',CANTIDAAUMENTO,@preciocompra,@stock,IDPRODUCTO,@preciocompra);
...
VALUES (DATE(CONVERT_TZ(NOW(),'SYSTEM','-05:00')),'SALIDA DIRECTA',CANTIDADISMINUIR,@preciocompra,@stock,IDPRODUCTO,@preciocompra);
...
VALUES (DATE(CONVERT_TZ(NOW(),'SYSTEM','-05:00')),'SALIDA DIRECTA',1,@preciocompra,@stock,ID_PRO,@preciocompra, IMEI_E);
...
VALUES (DATE(CONVERT_TZ(NOW(),'SYSTEM','-05:00')),'DEVOLUCION INSUMO',CANTD,@preciocompra,@stock,IDPRODUC,@preciocompra, CONCAT('R-000',RECEID), @NOMUSUARIO);
...
if @CANTIDAD = 0 THEN
INSERT into recep_equipo (rece_id, equipo, serie, monto, abono, fecha, falla)values(ID_RE, EQUI,SERI, MONT, ADELNT, DATE(CONVERT_TZ(NOW(),'SYSTEM','-05:00')), P_FALLA);
SELECT 1;
ELSE
SELECT 2;
END IF;
...
BEGIN
 ...
 -- Verificar si el IMEI ya está registrado
    ...
    ELSE
        ...
        VALUES (ID_P, IMEI_R, DATE(CONVERT_TZ(NOW(),'SYSTEM','-05:00')), 'No');
...
VALUES (DATE(CONVERT_TZ(NOW(),'SYSTEM','-05:00')),'INGRESO',1,@preciocompra,@stock,ID_P,@preciocompra, IMEI_R);
...
if @CANTIDAD = 0 THEN
	INSERT INTO caja (caja_descripcion, caja_monto_inicial, caja_fecha_ap, caja_estado, caja_hora_aper) VALUES(DESCRIPCION, MONTO_INI, DATE(CONVERT_TZ(NOW(),'SYSTEM','-05:00')), 'VIGENTE', TIME(CONVERT_TZ(NOW(),'SYSTEM','-05:00')));
SELECT 1;
ELSE
SELECT 2;
END IF;
...
caja_fecha_cie= DATE(CONVERT_TZ(NOW(),'SYSTEM','-05:00')), 
...
SET @CANTIDAD:=(SELECT COUNT(*) FROM cliente where cliente_dni=DNI );
IF @CANTIDAD = 0 THEN
	INSERT INTO cliente(cliente_nombres,cliente_celular,cliente_dni,cliente_fregistro,cliente_estado,cliente_direccion,cliente_ape_p,cliente_ape_m, cliente_correo, cliente_tipo_doc) VALUES(NOMBRE,CELULAR,DNI,DATE(CONVERT_TZ(NOW(),'SYSTEM','-05:00')),'ACTIVO',DIRECCION,			APE_P, APE_M, CORREO, TIPODOC);
	SELECT 1;
ELSE
	SELECT 2;
END IF;
...
BEGIN
INSERT INTO cotizacion_detalle(coti_id, producto_id,coti_detalle_cantidad,coti_detalle_precio,coti_detalle_estado,coti_detalle_fecha)VALUES(IDCOTI,PRODUCTO,CANTIDAD,PRECIO,'ACTIVO',DATE(CONVERT_TZ(NOW(),'SYSTEM','-05:00')));
...
VALUES(p_id_rece, p_id_insumo, p_cantid, p_monto, CONVERT_TZ(CURRENT_TIMESTAMP(),'SYSTEM','-05:00'), @preciocompra);
...
VALUES (DATE(CONVERT_TZ(NOW(),'SYSTEM','-05:00')),'SALIDA INSUMOS',p_cantid, @precioventa, @stock, p_id_insumo, @precioventa, CONCAT('R-000',p_id_rece), @NOMUSUARIO);
...
BEGIN
INSERT INTO producto_detalle(producto_id, imei, fecha, vendido)VALUES(IDPRO,PRODUCTO, DATE(CONVERT_TZ(NOW(),'SYSTEM','-05:00')), 'No');
...
BEGIN
INSERT INTO recep_equipo(rece_id, equipo, serie, monto, abono, fecha, falla)VALUES(IDRECE, EQUIP,SERI, MONT, ABON, DATE(CONVERT_TZ(NOW(),'SYSTEM','-05:00')), P_FALLA);
...
BEGIN
INSERT INTO detalle_venta(venta_id, producto_id,vdetalle_cantidad,vdetalle_precio,vdetalle_estado,vdetalle_fecha, v_imei, vdetalle_descuento)VALUES(IDVENTA,PRODUCTO,CANTIDAD,PRECIO,'VENDIDO',DATE(CONVERT_TZ(NOW(),'SYSTEM','-05:00')), P_IMEI, P_DESCUENTO);
...
VALUES(DATE(CONVERT_TZ(NOW(),'SYSTEM','-05:00')),'SALIDA',CANTIDAD,@precioventa,@stock,PRODUCTO,IDVENTA,@ID_DETALLE_VENTA,@COMPROBANTE,@preciocompra, P_IMEI );
		END
;;
delimiter ;
...
INSERT into gastos(gastos_descripcion,gastos_monto,gastos_responsable,gastos_fregistro,gastos_estado, estado_caja, tipo_mov)values(GASTO,NOMTO,RESPONSABLE,DATE(CONVERT_TZ(NOW(),'SYSTEM','-05:00')),'ACTIVO', 'ABIERTO', TIPO_M)
;;
delimiter ;
...
IF @CANTIDAD = 0 THEN
INSERT INTO recep_insumos (rece_id, producto_id, cantidad, fecha, monto_ri) VALUES (IDRECEP, IDPROD, CANTD, CONVERT_TZ(CURRENT_TIMESTAMP(),'SYSTEM','-05:00'), MONTOTOT );
...
insert into kardex (kardex_fecha, kardex_tipo, kardex_salida, kardex_p_salida, kardex_total, producto_id, kardex_precio_general, venta_comprobante, tecnico) 
VALUES (DATE(CONVERT_TZ(NOW(),'SYSTEM','-05:00')),'SALIDA INSUMOS',CANTD,@precioventa,@stock,IDPROD,@precioventa, CONCAT('R-000',IDRECEP), @NOMUSUARIO);
...
INSERT INTO notas(descripcion, estado,  fecha, usu_id ) VALUES(p_descrip, '1', CONVERT_TZ(CURRENT_TIMESTAMP(),'SYSTEM','-05:00'), p_idusuario);
...
SET @CANTIDAD:=(SELECT COUNT(*) FROM proveedor where prove_ruc=RUC);
if @CANTIDAD = 0 THEN
INSERT into proveedor(prove_ruc,prove_razon,prove_direccion,prove_celular,prove_fregistro,prove_estado)values(RUC,RAZON,DIRECCION,CELULAR,DATE(CONVERT_TZ(NOW(),'SYSTEM','-05:00')),'ACTIVO');
SELECT 1;
ELSE
SELECT 2;
END IF;
...
INSERT INTO recepcion(cliente_id, motivo_id, rece_monto, rece_fregistro, rece_estado, rece_estatus, rece_adelanto, rece_debe, rece_concepto,rece_fentrega, rece_cod, marca_id, tecnico, usuario_registrador, rece_foto1)
	VALUES(IDCLIENTE,   IDMOTIVO, MONTO,DATE(CONVERT_TZ(NOW(),'SYSTEM','-05:00')),'EN REPARACION','ACTIVO',ADELANTO,DEBE,ACCESORIOS,FENTREGA, COD_REC, 1, TECNICOID, USUA_ID, RUTA);
	 SELECT LAST_INSERT_ID();
...
-- 	VALUES(LAST_INSERT_ID() , MONTO, ACCESORIOS, 'CARLOS', DATE(CONVERT_TZ(NOW(),'SYSTEM','-05:00')), 'EN REPARACION', 'ACTIVO', 'ABIERTO', @cajaid, TECNICOID );
	END
;;
delimiter ;
...
SET @CANTIDAD:=(SELECT COUNT(*) FROM rol where rol_nombre=ROL);
if @CANTIDAD = 0 THEN
INSERT into rol(rol_nombre,rol_fregistro,rol_estado)values(ROL,DATE(CONVERT_TZ(NOW(),'SYSTEM','-05:00')),'ACTIVO');
SELECT 1;
ELSE
SELECT 2;
END IF;
...
VALUES(IDRECE,MONTO,CONCEPTO,RESPONSABLE,COMENTARIO,DATE(CONVERT_TZ(NOW(),'SYSTEM','-05:00')),ESTADOSERV,'ACTIVO', 'ABIERTO', OBSERVA, MODELO, IDFORMA_P, MONTO_EFEC, COD_OPERA, MONTO_TARJ, IDCAJA, IDTECNICO);
...
VALUES (IDCLIENTE,COMPROBANTE,SERIE,NUMERO,DATE(CONVERT_TZ(NOW(),'SYSTEM','-05:00')),IMPUESTO,TOTAL,'PAGADA',IDCOMPROBANTE,PORCENTAJE,IDUSUARIO,TIME(CONVERT_TZ(NOW(),'SYSTEM','-05:00')),IDFPAGO,OBSERVA, 'ABIERTO', MONT_EFECT, COD_OPERAC, MONTO_TARJET, IDCAJA, P_DESCUENTO);
SELECT LAST_INSERT_ID();
...
FROM
	...
	and (select MONTH(gastos_fregistro))=MES
		and YEAR(gastos_fregistro)=YEAR(DATE(CONVERT_TZ(NOW(),'SYSTEM','-05:00')))
		ORDER BY gastos_id DESC
;;
delimiter ;
...
FROM
	...
	ON 
		...
		and YEAR(servicio_fregistro)=YEAR(DATE(CONVERT_TZ(NOW(),'SYSTEM','-05:00')))
